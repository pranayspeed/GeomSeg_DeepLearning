...
class Options():
    ...
    def initialize(self):
        ...
        self.parser.add_argument('--dataset', type=str, default='oxford', help='kitti')
        self.parser.add_argument('--dataroot', default='/ssd/jiaxin/USIP_datasets/oxford', help='path to images & laser point clouds')
        ...
        self.parser.add_argument('--input_pc_num', type=int, default=16384, help='# of input points')
        ...
        self.parser.add_argument('--node_num', type=int, default=128, help='som node number')
        self.parser.add_argument('--k', type=int, default=1, help='k nearest neighbor')
        self.parser.add_argument('--node_knn_k_1', type=int, default=16, help='k nearest neighbor of SOM nodes searching on SOM nodes')
        ...
        self.parser.add_argument('--rot_horizontal', type=bool, default=True, help='Rotation augmentation around vertical axis.')
        self.parser.add_argument('--rot_3d', type=bool, default=False, help='Rotation augmentation around xyz axis.')
        ...
        self.parser.add_argument('--positive_radius_threshold', type=float, default=5, help='Max threshold to find positive scans.')
        self.parser.add_argument('--negative_radius_threshold', type=float, default=50, help='Min threshold to find negative scans.')

        # descriptor
        self.parser.add_argument('--ball_radius', type=int, default=2, help='Ball query around each keypoint')
        self.parser.add_argument('--ball_nsamples', type=int, default=64, help='Ball query around each keypoint')
        self.parser.add_argument('--descriptor_len', type=float, default=128, help='length of descriptor')

        # triplet loss
        self.parser.add_argument('--sigma_max', type=float, default=3, help='same as detector option keypoint_outlier_thre')
        self.parser.add_argument('--triple_loss_gamma', type=float, default=0.5, help='')

        # oxford dataset configuration
        self.parser.add_argument('--is_height_scaling', type=bool, default=True, help='Height scaling [0.25, 1.2]')

        # for detector
        self.parser.add_argument('--loss_sigma_lower_bound', type=float, default=0.001, help='Sigma lower bound')
