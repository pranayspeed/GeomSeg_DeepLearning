...
class RandLANet_with_encoder(nn.Module):
    def __init__(self, d_in, num_classes, num_neighbors=16, decimation=4, device=torch.device('cpu')):
        super(RandLANet, self).__init__()
        # self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.num_neighbors = num_neighbors
    ...
    def forward(self, input):
        ...
        encoded_out = x
        print("\nMLP Encoder out shape: ", encoded_out.shape, N//decimation_ratio)
        encoded_out = encoded_out[:,:,torch.argsort(permutation[:N//decimation_ratio])]
