...
def header_properties(field_list, field_names):
    ...
    for fields in field_list:
        for field in fields.T:
            lines.append('property %s %s' % (field.dtype, field_names[i]))
            i += 1
...
def write_ply(filename, field_list, field_names):
    ...
    for i, field in enumerate(field_list):
        if field is None:
            ...
            print(field.shape, "Reshaped")
            field_list[i] = field.reshape(-1, 1)

    # check all fields have the same number of data
    print(field_list[0].shape, field_list[1].shape)
    n_points = [field.shape[0] for field in field_list]
    print(n_points, n_points[0])
    ...
    with open(filename, 'ab') as plyfile:
        ...
        for fields in field_list:
            for field in fields.T:
                type_list += [(field_names[i], str(field.dtype))]
                i += 1
