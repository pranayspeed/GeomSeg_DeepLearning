...
import os
import torch
from models.kpconv_panoptic import PanopticKPConv

from omegaconf import OmegaConf
import numpy as np

from points3d.dataset.bagsfit import BagsfitPanopticDataset
import time
...
dataset = BagsfitPanopticDataset(params)
...
######################################
# Create a model
model = PanopticKPConv(cat_to_seg, num_classes=num_classes, remapping_map=remapping_map, device="cpu")
...
with tqdm(test_loader) as tq_test_loader:
    for i, data in enumerate(tq_test_loader):
        ...
        pred = torch.argmax(output.semantic_logits, dim=1)
        instance_pred = output.clusters
