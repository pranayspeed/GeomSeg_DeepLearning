...
from points3d.dataset.bagsfit_config import *
...
class SampledBagsfit(Dataset):
    ...
    def process_one(scan_file, label_file, transform, out_file):
        ...
        print("Processed file %s, nb points = %i", os.path.basename(out_file), data.pos.shape[0])
...
class BagsfitDataset(BaseDataset):
    ...
    def _inverse_remap_labels(self, semantic_label):
        """ Remaps labels to [0 ; num_labels -1]. Can be overriden.
        """
        new_labels = semantic_label.clone()
        for source, target in self.LEARNING_MAP_INV.items():
            mask = semantic_label == source
            new_labels[mask] = target
...
class BagsfitPanopticDataset(BagsfitDataset):
